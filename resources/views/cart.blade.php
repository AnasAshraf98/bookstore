                                    ...
                                    @foreach ($items as $item)
                                        ...
                                        <tbody>
                                            <tr>
                                                ...
                                                <td>
                                                    <form style="float: left;margin: auto 5px" action="{{route('cart.remove_all',$item->id)}}" method="post">
                                                        @csrf
                                                        <button type="submit" class="btn btn-danger btn-sm">أزل الكل</button>
                                                    </form>
                                                    
                                                    <form style="float: left;margin: auto 5px" action="{{route('cart.remove_one',$item->id)}}" method="post">
                                                        @csrf
                                                        <button type="submit" class="btn btn-warning btn-sm">أزل واحدا</button>
                                                    </form>
                                                </td>
                                            </tr>
                                        </tbody>
